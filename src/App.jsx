...
const PAIN_WORDS=['dor','dói','doendo','machucou','machucar','lesão','lesão','ardendo','inflamado','travado','travei','torci','torção','queimou','queimando','incômodo','incomoda','desconforto'];
const hasPain = (log) => {
  const all = [log.notes||'', ...(log.exercises||[]).flatMap(e=>[...( e.sets||[]).map(s=>s.notes||'')])].join(' ').toLowerCase();
  return PAIN_WORDS.some(w=>all.includes(w));
};
...
const Tab=({active,onClick,children})=>(
  <button onClick={onClick} style={{padding:'6px 11px',border:`1px solid ${active?ACC:BDR}`,background:active?'rgba(141,198,63,0.08)':'transparent',color:active?ACC:MUTED,borderRadius:5,cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:"'Outfit',sans-serif"}}>
    {children}
  </button>
);
const Spin=()=>(
...
const StatBox=({label,value,unit='',color=ACC})=>(
  <div style={{background:SURF2,border:`1px solid ${BDR}`,borderRadius:8,padding:'12px 14px',textAlign:'center'}}>
    <div style={{fontFamily:"'Bebas Neue'",fontSize:26,color,lineHeight:1}}>{value||'—'}{unit&&<span style={{fontSize:14}}> {unit}</span>}</div>
    <div style={{fontSize:10,color:MUTED,textTransform:'uppercase',letterSpacing:1,marginTop:4}}>{label}</div>
  </div>
);
...
export default function App(){
  ...
  const loadCoach=async()=>{
    ...
    students.forEach(s=>{
      data[s.name]=(allLogs).filter(l=>l.student_id===s.name);
      profs[s.name]=s;
      wls[s.name]=(allWL).filter(l=>l.student_id===s.name);
    });
    setStudData(data);setStudProfiles(profs);setStudWeightLogs(wls);
    setLoading(false);
  };
  ...
  const saveProfile=async(data)=>{
    setLoading(true);
    await sb.upsert('students',{id:student,name:student,...data});
  ...
  const addWeightLog=async()=>{
    ...
    await loadWeightLogs(student);await loadProfile(student);
    setNewWeight('');setNewBF('');setLoading(false);
  };
  ...
  const progressData=useMemo(()=>{
    ...
    return[...logs].reverse().filter(l=>l.exercises?.some(e=>e.name===selEx)).map(l=>{
      ...
      return{date:l.date.slice(5),maxLoad:done.length?Math.max(...done.map(s=>parseFloat(s.load)||0)):0,volume:Math.round(done.reduce((a,s)=>a+(parseFloat(s.reps)||0)*(parseFloat(s.load)||0),0))};
    });
  },[logs,selEx]);
  const stats=useMemo(()=>{
  ...
  const exSum=(ex)=>{const done=ex.sets.filter(s=>s.completed);return{done:done.length,total:ex.sets.length,maxLoad:done.length?Math.max(...done.map(s=>parseFloat(s.load)||0)):0,vol:Math.round(done.reduce((a,s)=>a+(parseFloat(s.reps)||0)*(parseFloat(s.load)||0),0))};};
  ...
  return(
    <div style={st.app}>
      ...
      <div style={st.main}>
        ...
        {/* PROFILE SETUP */}
        {view==='profile-setup'&&(
          <ProfileSetup name={student} onSave={saveProfile} loading={loading} styles={{st,ACC,MUTED,BDR,SURF,SURF2,TEXT}}/>
        )}
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div>
                    <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:4,flexWrap:'wrap'}}>
                      ...
                      {hasPain(l)&&<span style={st.tagD}>⚠️ Dor reportada</span>}
                    </div>
        ...
        {view==='perfil'&&(
          <div>
            ...
            {/* FIFA Card */}
            <div style={{background:`linear-gradient(135deg, #0a1a00 0%, #1a3300 50%, #0a1a00 100%)`,border:`2px solid ${ACC}`,borderRadius:16,padding:'28px 24px',marginBottom:20,position:'relative',overflow:'hidden'}}>
              ...
              <div style={{position:'absolute',bottom:0,left:0,width:150,height:150,background:'rgba(141,198,63,0.03)',borderRadius:'50%',transform:'translate(-50px,50px)'}}/>
              <div style={{display:'flex',gap:24,alignItems:'flex-start',flexWrap:'wrap'}}>
                <div style={{flex:1,minWidth:200}}>
                  <div style={{fontFamily:"'Bebas Neue'",fontSize:11,letterSpacing:4,color:MUTED,marginBottom:4}}>PŌKAI MOVEMENT</div>
                  <div style={{fontFamily:"'Bebas Neue'",fontSize:36,letterSpacing:2,color:TEXT,lineHeight:1,marginBottom:4}}>{student}</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,minWidth:220}}>
                  <StatBox label="Altura" value={profile?.height} unit="cm"/>
                  <StatBox label="Peso atual" value={profile?.weight} unit="kg"/>
                  <StatBox label="% Gordura" value={profile?.body_fat} unit="%" color={WARN}/>
                  <StatBox label="Treinos/semana" value={stats.lastWeek} color='#5BC8F5'/>
                </div>
              </div>
            </div>
            ...
            {weightLogs.length>1&&(
              <div style={st.sect}>
                ...
                {weightLogs.some(l=>l.body_fat)&&(
                  <>
                    <span style={{...st.sectT,marginTop:16}}>EVOLUÇÃO DO % GORDURA</span>
                    <ResponsiveContainer width="100%" height={180}>
                      <LineChart data={weightLogs.filter(l=>l.body_fat)} margin={{top:5,right:10,left:0,bottom:5}}>
                        <CartesianGrid strokeDasharray="3 3" stroke={BDR}/>
                        <XAxis dataKey="date" tick={{fill:MUTED,fontSize:10}} tickFormatter={d=>d.slice(5)}/>
                        <YAxis tick={{fill:MUTED,fontSize:11}} unit="%" width={40} domain={['auto','auto']}/>
                        <Tooltip contentStyle={{background:SURF2,border:`1px solid ${BDR}`,borderRadius:6,color:TEXT,fontSize:12}}/>
                        <Line type="monotone" dataKey="body_fat" name="% Gordura" stroke={WARN} strokeWidth={2} dot={{fill:WARN,r:3}}/>
                      </LineChart>
                    </ResponsiveContainer>
                  </>
                )}
            ...
            {weightLogs.length>0&&(
              <div style={st.sect}>
                <span style={st.sectT}>HISTÓRICO DE MEDIDAS</span>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead><tr>{['Data','Peso','% Gordura'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {[...weightLogs].reverse().slice(0,10).map((l,i)=>(
                      <tr key={i}>
                        <td style={st.td}>{l.date}</td>
                        <td style={{...st.td,color:ACC,fontFamily:"'JetBrains Mono',monospace"}}>{l.weight?l.weight+' kg':'—'}</td>
                        <td style={{...st.td,color:WARN,fontFamily:"'JetBrains Mono',monospace"}}>{l.body_fat?l.body_fat+'%':'—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:MUTED,background:SURF2,padding:'4px 10px',borderRadius:4,display:'inline-flex',gap:8}}>
                      <span>📋</span><span style={{color:ACC,fontFamily:"'JetBrains Mono',monospace"}}>{ex.planned.sets}x{ex.planned.reps}{ex.planned.load?' @ '+ex.planned.load:''}</span>
                    </div>
                      ...
                      {ex.sets.map((set,si)=>(
                        <tr key={si} style={{opacity:set.completed?1:0.5}}>
                          ...
                          <td style={{...st.td,textAlign:'center'}}>
                            <div onClick={()=>updSet(ei,si,'completed',!set.completed)} style={{width:24,height:24,borderRadius:5,cursor:'pointer',margin:'0 auto',border:`2px solid ${set.completed?ACC:'#444'}`,background:set.completed?ACC:'transparent',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,color:'#000'}}>{set.completed?'✓':''}</div>
                          </td>
            ...
            {logs.map(l=>(
              <div key={l.id} style={st.card}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${BDR}`}}>
                  ...
                  <div style={{display:'flex',gap:6}}>{l.rpe&&<span style={st.tag}>RPE {l.rpe}</span>}<span style={{...st.tag,background:'rgba(255,255,255,0.05)',color:MUTED}}>⚡{l.energy}</span></div>
                </div>
                <div style={{overflowX:'auto'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',minWidth:420}}>
                    <thead><tr>{['Exercício','Planejado','Séries ok','Melhor carga','Volume'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr></thead>
                    <tbody>
                      {l.exercises?.filter(e=>e.name).map((ex,i)=>{const{done,total,maxLoad,vol}=exSum(ex);return(
                        <tr key={i}>
                ...
                {/* Pain details */}
                {l.exercises?.some(e=>e.sets?.some(s=>s.notes&&PAIN_WORDS.some(w=>s.notes.toLowerCase().includes(w))))&&(
                  <div style={{marginTop:10,background:'rgba(224,80,80,0.06)',border:`1px solid rgba(224,80,80,0.2)`,borderRadius:6,padding:'10px 14px'}}>
                    <div style={{fontSize:11,color:DANGER,fontWeight:700,marginBottom:6,letterSpacing:1}}>⚠️ DORES REPORTADAS</div>
                    {l.exercises.map((ex,i)=>ex.sets?.map((s,j)=>s.notes&&PAIN_WORDS.some(w=>s.notes.toLowerCase().includes(w))&&(
                      <div key={`${i}-${j}`} style={{fontSize:12,color:'#CCC',marginBottom:3}}>
                        <span style={{color:MUTED}}>{ex.name} série {j+1}:</span> {s.notes}
                      </div>
                    )))}
                    {l.notes&&PAIN_WORDS.some(w=>l.notes.toLowerCase().includes(w))&&(
                      <div style={{fontSize:12,color:'#CCC',marginTop:4}}><span style={{color:MUTED}}>Obs sessão:</span> {l.notes}</div>
                    )}
                  </div>
                )}
                ...
                {selEx&&progressData.length>0&&(
                  <>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
                      {[['Carga máxima',progressData[progressData.length-1]?.maxLoad?progressData[progressData.length-1].maxLoad+' kg':'—'],['Melhor volume',Math.max(...progressData.map(d=>d.volume))+' kg'],['Sessões',progressData.length]].map(([k,v])=>(
                    ...
                    <div style={st.sect}>
                      ...
                      <ResponsiveContainer width="100%" height={180}><LineChart data={progressData} margin={{top:5,right:10,left:0,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke={BDR}/><XAxis dataKey="date" tick={{fill:MUTED,fontSize:11}}/><YAxis tick={{fill:MUTED,fontSize:11}} unit="kg" width={52}/><Tooltip contentStyle={{background:SURF2,border:`1px solid ${BDR}`,borderRadius:6,color:TEXT,fontSize:12}}/><Line type="monotone" dataKey="volume" name="Volume" stroke={WARN} strokeWidth={2} dot={{fill:WARN,r:3}}/></LineChart></ResponsiveContainer>
                    </div>
                  </>
                )}
        ...
        {view==='coach'&&(
          <div>
            <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:22}}>
              <h1 style={{fontFamily:"'Bebas Neue'",fontSize:30,letterSpacing:3,color:ACC,margin:0}}>PŌKAI — PAINEL DO PROFESSOR</h1>
              ...
              <>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:20}}>
                  {[
                    ...
                    ['Alertas de dor',Object.values(studData).reduce((a,ls)=>a+ls.filter(l=>hasPain(l)).length,0)],
                  ].map(([k,v],i)=>(
                    <div key={k} style={{background:SURF,border:`1px solid ${i===2&&v>0?DANGER:BDR}`,borderRadius:8,padding:'12px 16px',textAlign:'center'}}>
                      <div style={{fontFamily:"'Bebas Neue'",fontSize:28,color:i===2&&v>0?DANGER:ACC}}>{v}</div>
                ...
                {allStudents.map(name=>{
                  ...
                  const prs={};
                  sLogs.forEach(l=>l.exercises?.forEach(e=>{if(!e.name)return;const max=Math.max(...e.sets.filter(s=>s.completed&&s.load).map(s=>parseFloat(s.load)||0),0);if(!prs[e.name]||max>prs[e.name])prs[e.name]=max;}));
                      ...
                      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',cursor:'pointer'}} onClick={()=>setExpandedStudent(open?null:name)}>
                        <div>
                          ...
                          <div style={{fontSize:12,color:MUTED,display:'flex',gap:12,flexWrap:'wrap'}}>
                            ...
                            <span>🔥 {lastWeek}/semana</span>
                            {sProf.age&&<span>👤 {sProf.age} anos</span>}
                            {sProf.weight&&<span>⚖️ {sProf.weight}kg</span>}
                            {sProf.body_fat&&<span>📊 {sProf.body_fat}% gord.</span>}
                        ...
                        <div style={{marginTop:16,borderTop:`1px solid ${BDR}`,paddingTop:16}}>
                          ...
                          {painLogs.length>0&&(
                            <div style={{background:'rgba(224,80,80,0.06)',border:`1px solid rgba(224,80,80,0.2)`,borderRadius:8,padding:'14px 16px',marginBottom:14}}>
                              <div style={{fontSize:11,color:DANGER,fontWeight:700,marginBottom:10,letterSpacing:1}}>⚠️ HISTÓRICO DE DORES ({painLogs.length} sessões)</div>
                              {painLogs.slice(0,3).map((l,i)=>(
                                <div key={i} style={{marginBottom:8,paddingBottom:8,borderBottom:i<Math.min(painLogs.length,3)-1?`1px solid rgba(224,80,80,0.15)`:'none'}}>
                                  ...
                                  {l.exercises?.map((ex,ei)=>ex.sets?.map((s,si)=>s.notes&&PAIN_WORDS.some(w=>s.notes.toLowerCase().includes(w))&&(
                                    <div key={`${ei}-${si}`} style={{fontSize:12,color:'#CCC'}}><span style={{color:MUTED}}>{ex.name} S{si+1}:</span> {s.notes}</div>
                                  )))}
                          ...
                          {sWL.length>1&&(
                            <div style={{background:SURF2,borderRadius:8,padding:'14px 16px',marginBottom:14}}>
                              <div style={{fontSize:11,color:MUTED,letterSpacing:1,marginBottom:10,textTransform:'uppercase'}}>Evolução do Peso</div>
                              <ResponsiveContainer width="100%" height={120}>
                                <LineChart data={sWL} margin={{top:2,right:8,left:0,bottom:2}}>
                                  ...
                                  <Line type="monotone" dataKey="weight" stroke={ACC} strokeWidth={2} dot={{fill:ACC,r:3}}/>
                                </LineChart>
                              </ResponsiveContainer>
                            </div>
                          )}
...
function ProfileSetup({name,onSave,loading,styles}){
  const{st,ACC,MUTED,BDR,SURF,SURF2}=styles;
  ...
  return(
    <div style={{maxWidth:500,margin:'0 auto'}}>
      <div style={{textAlign:'center',marginBottom:32}}>
        ...
        <p style={{color:MUTED,fontSize:14}}>Olá, <strong style={{color:'#FFF'}}>{name}</strong>! Complete seu perfil para começar.</p>
      </div>
      <div style={st.sect}>
        ...
        <div style={st.g2}>
          ...
          <div style={st.fld}><label style={st.lbl}>% Gordura corporal</label><input type="number" step="0.1" placeholder="Ex: 18.5 (opcional)" style={st.inp} value={form.body_fat} onChange={e=>upd('body_fat',e.target.value)}/></div>
        </div>
        <p style={{fontSize:11,color:MUTED,marginTop:4}}>* Você poderá atualizar esses dados a qualquer momento no seu perfil.</p>
      </div>
